...
router.post("/generate", async (req, res) => {
  try {
    ...
    if (existingSummary) {
      return res.status(200).json({ message: "Summary already exists", summary: existingSummary });
    }
    ...
    thoughts.forEach(t => {
      if (!continentData[t.continent]) continentData[t.continent] = { count: 0, moods: {} };
      continentData[t.continent].count++;
      continentData[t.continent].moods[t.mood] = (continentData[t.continent].moods[t.mood] || 0) + 1;

      if (!countryData[t.country]) countryData[t.country] = { count: 0, moods: {} };
      countryData[t.country].count++;
      countryData[t.country].moods[t.mood] = (countryData[t.country].moods[t.mood] || 0) + 1;

      const cityKey = `${t.city}_${t.country}`;
      if (!cityData[cityKey]) cityData[cityKey] = { city: t.city, country: t.country, count: 0, moods: {} };
      cityData[cityKey].count++;
      cityData[cityKey].moods[t.mood] = (cityData[cityKey].moods[t.mood] || 0) + 1;
    });
